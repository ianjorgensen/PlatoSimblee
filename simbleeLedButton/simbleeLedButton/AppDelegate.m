...
- (void)bleUnsupported
{
    [self alert:@"BLE Unsupported" title:@"Plato" block:nil];
}

- (void)bleUnauthorized
{
    [self alert:@"BLE Unauthorized" title:@"Plato" block:nil];
}

- (void)blePoweredOff
{
    [self alert:@"BLE Powered Off.\n\nOpen Settings, and switch Bluetooth On." title:@"Plato" block:nil];
}
...
- (void)scanViewController:(ScanViewController *)scanViewController didDiscoverSimblee:(Simblee *)simblee
{
    ...
    if (_backgroundMode && !active && simblee.advertisementPackets == 1) {
        UILocalNotification *notify = [[UILocalNotification alloc] init];
        notify.alertBody = @"Plato Discovered";
        [[UIApplication sharedApplication] presentLocalNotificationNow:notify];
    }
}
